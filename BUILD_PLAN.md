...
**Today's next action:** Marble is **under review** in the community marketplace — actually submitted 6/10/26 via the Claude **Console** (`platform.claude.com/plugins/submit`), NOT 5/31 (that earlier attempt never registered — the Console "Plugin submissions" page showed "No submissions yet", so the real clock started 6/10). Check status ~week of 6/17/26: (a) the Console submissions page will flip to approved/rejected, and (b) once approved it appears in `github.com/anthropics/claude-plugins-community/blob/main/.claude-plugin/marketplace.json` (nightly sync). When approved → live smoke test: `/plugin marketplace add anthropics/claude-plugins-community` then `/plugin install marble@claude-community` in a fresh Claude Code. If rejected → read the reason; most likely culprit is shipping the dev-workspace files (CLAUDE.md/BUILD_PLAN.md/BACKLOG.md) inside the plugin, fix = exclude those three. Correct submission URL for an individual author is the Console one, NOT `claude.ai/settings/plugins/submit` (deprecated, redirects to the personal-skills page).
